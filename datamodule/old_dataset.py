...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.mfcc_sample_rate = dataset_config.get("mfcc_sample_rate", 16000)
    ...
    def extract_mfcc(self, audio_raw):
        """
        Extract MFCC features from raw audio.
        Args:   
            audio_raw (np.ndarray): Raw audio waveform.
        Returns:
            torch.Tensor: MFCC features.
        """
        mfcc = librosa.feature.mfcc(
            y=audio_raw,
            sr=self.mfcc_sample_rate,
            n_mfcc=self.n_mfcc,
            n_fft=400, # Match whisper's FFT window size
            hop_length=160, # Match whisper's hop length
        )

        # Transpose to (time, features) to match mel spectrogram format
        mfcc = torch.from_numpy(mfcc.T).float()  # (time, n_mfcc)
        return mfcc
    ...
    def __getitem__(self, index):
        ...
        if self.input_type == "raw":
            ...
            audio_raw = whisper.pad_or_trim(audio_raw)
            audio_mel = self.extract_mfcc(audio_raw)  # reuse mel variable name 
            audio_length = (audio_mel.shape[0] + 1) // 2  # ad-hoc for whisper for 2x downsample from mel to feats
            audio_length = audio_length // 5 # ad-hoc for 5x fc downsample

        elif self.input_type == "combined":
            raise NotImplementedError("Combined input type is not implemented yet.")
